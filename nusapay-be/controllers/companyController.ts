...
export async function loadCompanyTransactionHistory(req: Request, res: Response){
  const { companyId } = req.body;

  try {
    const latestTransactionHistory = await TransactionRecordModel.find({
      companyId,
    })
      .sort({ timestamp: -1 }) // descending (terbaru di atas)
      .lean(); // supaya hasilnya plain JS object dan lebih cepat
...
export async function loadDetailedTransactionHistory(req: Request, res: Response){
  const { companyId, txHash } = req.body;

  try {
    const latestTransactionHistory = await TransactionRecordModel.findOne({
      companyId, txHash
    })

    res.status(201).json({
      message: "History of transaction detail successfully sended",
      data: latestTransactionHistory,
    });
    return;
    ...
    res.status(500).json({
      message: "History of transaction detail failed to send",
      error: err.message,
    });
    return;
  }
...
export async function loadDetailedEmployeeTransactionHistory(req: Request, res: Response){
  const {txId} = req.body;
  ...
  try {
    ...
    res.status(201).json({
      message: "History of employee's transaction detail successfully sended",
      data: latestTransactionHistory,
    });
    return;
    ...
    res.status(500).json({
      message: "History of employee's transaction detail failed to send",
      error: err.message,
    });
    return;
  }
